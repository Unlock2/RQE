	          ...
	          -->
				<div class="search">
					<form id="search-bmt">
						...
						<c:choose>
							<c:when test="${au eq '관리자'}">
								<a href="javascript:deleteNewList()" class="btnNewSave"
									id="btnNewSave" style="float: right;"> <img
									src="../images/board/btn_dele.gif" alt="newSave" />
								</a>

								<a href="javascript:saveNewList()" class="btnNewSave"
									id="btnNewSave" style="float: right;"> <img
				...
				<div>
<!-- 					<button type="button" id="checkAll" style="float:right; margin-left:2%">전체선택</button> -->
					<c:set value="<%=au %>" var="au" />
					<c:choose>
						<c:when test="${au eq '관리자'}">
							...
							<a href="javascript:removeNewList()" class=""
	...
	*/
	...
	getList = function() {
		...
		$.ajax({
			...
			, success : function(data, stat, xhr) {
				...
				for(var i = 1; i <= lastBmtIndex; i++) {
					var dataListBmtId = data.list[dataIndex].bmtid;
					var dataListBmtIndex = dataListBmtId.split('-')[1];
					
					
					if(dataListBmtIndex == i) {
						html += '<tr>';		
						html += '<td><input type="checkbox" name="chkBmtId" value="' + dataList[dataIndex].bmtid +'||'+dataList[dataIndex].userid+ '"/></td>';
						html += '<td><input type="text" id="bmtid_' + i + '" value = "' + dataList[dataIndex].bmtid + '" readonly style="text-align: center;"/></td>'
						html += '<td><input type="text" id="startpoi_' + i + '" value = "' + dataList[dataIndex].startpoi + '" readonly  style="width: 506px;text-align: center;"/></td>'
						html += '<td><input type="text" id="endpoi_' + i + '" value = "' + dataList[dataIndex].endpoi + '" readonly  style="width: 506px;text-align: center;"/></td>'
						...
						html += '<tr>';		
						html += '<td></td>';
						html += '<td><input type="text" id="bmtid_' + i + '" value = "' + new Date(date).format('yyMMdd') + '-'+pad(i,2) + '" readonly/></td>'
	...
	deleteNewList = function(){
		var deleteArray = [];
		 ...
		 addRemoveIndex = addRemoveIndex - deleteArray.length;
	...
	saveNewList = function(){
		...
		var saveData = [];
		for(var i = 1 ; i <= addRemoveIndex; i++){
	...
	addNewList = function() {
		...
		html += '<tr>';
		html += '<td></td>';
		html += '<td><input type="text" id="bmtid_' + addRemoveIndex + '" value = "' + new Date(date).format('yyMMdd') + '-'+pad(addRemoveIndex,2) + '" readonly  style="text-align: center;"/></td>'
		html += '<td><input type="text" id="startpoi_' + addRemoveIndex + '" value = ""  style="width: 506px;text-align: center;"/></td>'
		html += '<td><input type="text" id="endpoi_' + addRemoveIndex + '" value = ""  style="width: 506px;text-align: center;"/></td>'
