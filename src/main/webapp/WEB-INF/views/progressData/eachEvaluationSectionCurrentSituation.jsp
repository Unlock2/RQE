                            ...
                            <tbody>
                            	...
                            	<c:forEach items="${list}" var="list">
                            		<tr>
	                            		<td>${list.bmtid }</td>
	                            		<c:choose>
	                            			<c:when test="${list.cp eq '평가 준비중'}">
	                            				<td style="color: #0000FF;">${list.bmt_start_tm }</td>
	                            			</c:when>
	                            			<c:when test="${list.cp eq '평가 진행중'}">
	                            		...
	                            		<c:choose>
	                            			<c:when test="${list.carinfo eq '평가 준비중'}">
	                            				<td style="color: #0000FF;">${list.bmt_start_tm }</td>
	                            			</c:when>
	                            			<c:when test="${list.carinfo eq '평가 진행중'}">
	                            				<td style="color: #FF0000;">${list.bmt_start_tm }</td>
	                            			</c:when>
	                            			<c:otherwise>
	                            				<td>${list.carinfo }</td>
	                            		...
	                            		<td>${list.endpoi }</td>
	                            		<c:choose>
	                            			<c:when test="${list.real_req_time eq '평가 준비중'}">
	                            				<td style="color: #0000FF;">${list.bmt_start_tm }</td>
	                            			</c:when>
	                            			<c:when test="${list.real_req_time eq '평가 진행중'}">
	                            				<td style="color: #FF0000;">${list.bmt_start_tm }</td>
	                            			</c:when>
	                            			<c:otherwise>
	                            		...
	                            		<c:choose>
	                            			<c:when test="${list.est_time eq '평가 준비중'}">
	                            				<td style="color: #0000FF;">${list.bmt_start_tm }</td>
	                            			</c:when>
	                            			<c:when test="${list.est_time eq '평가 진행중'}">
	                            				<td style="color: #FF0000;">${list.bmt_start_tm }</td>
	                            			</c:when>
	                            			<c:otherwise>
	                            				<td>${list.est_time }</td>
	                            		...
	                            		<c:choose>
	                            			<c:when test="${list.est_distance eq '평가 준비중'}">
	                            				<td style="color: #0000FF;">${list.bmt_start_tm }</td>
	                            			</c:when>
	                            			<c:when test="${list.est_distance eq '평가 진행중'}">
	                            				<td style="color: #FF0000;">${list.bmt_start_tm }</td>
	                            			</c:when>
	                            			<c:otherwise>
	                            		...
	                            		<c:choose>
	                            			<c:when test="${list.est_charge eq '평가 준비중'}">
	                            				<td style="color: #0000FF;">${list.bmt_start_tm }</td>
	                            			</c:when>
	                            			<c:when test="${list.est_charge eq '평가 진행중'}">
	                            				<td style="color: #FF0000;">${list.bmt_start_tm }</td>
	                            			</c:when>
	                            			<c:otherwise>
