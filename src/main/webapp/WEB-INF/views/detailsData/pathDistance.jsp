...
<script>
	...
	function pathDistanceSearch() {
		...
		$.ajax({
			url : '/detailsData/pathDistanceData',
			type : 'GET',
			dataType : 'json',
			data : params,
			...
			success : function(data, status, xhr) {
				...
				data.list.forEach(function(items) {
					var html = [ '<tr class="',  items.check_select , '">',
							'<td>', items.cp, '</td>', '<td>',
							items.sum_distance + 'km', '</td>', '<td>',
							items.sub_distance + 'km', '</td>', '<td>',
							items.ratio_distance + '%', '</td>', '</tr>' ]
							.join('');
					list.append(html);
				});
				//강조 표시
	...
	function ratioDistanceChart(dataList) {
		...
		if (dataList.length != 0) {
			for (var i = 0; i < dataList.length; i++) {
				...
				ratioDistance.push(dataList[i].ratio_distance);
			}
		}
