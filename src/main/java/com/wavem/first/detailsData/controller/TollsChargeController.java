...
@Controller("TollsChargeController")
public class TollsChargeController {
		private static final Logger logger = LoggerFactory.getLogger(TollsChargeController.class);
		
		@Resource(name="TollsChargeService")
		...
		@ResponseBody
		@RequestMapping(value = "/detailsData/tollsChargeData", method = RequestMethod.GET)
		public ModelAndView getPathDistanceData(HttpServletRequest request, HttpSession session) {
			...
			List<Map<String, Object>> list = tollsChargeService.getTollsChargeData(map);
			List<Map<String, Object>> chartList = tollsChargeService.getTollsChargeChartData(map);
